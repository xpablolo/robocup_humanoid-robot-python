...
class MyAgent(SparkAgent):
    def think(self, perception):
        ...
        # perception.joint: dicc with key "hj1" for HeadYaw (see JOINT_SENSOR_NAMES in spark_agent.py)
        if "hj1" in perception.joint:
            angle = perception.joint["hj1"]
        # perception.joint_temperature: dicc with key "hj1" for HeadYaw (see JOINT_SENSOR_NAMES in spark_agent.py)
        if "hj1" in perception.joint_temperature["hj1"]
